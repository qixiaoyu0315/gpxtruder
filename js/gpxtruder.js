...
var submitInput = function () {
	...
	var validOptions = function (options) {
		...
		if (options.regionfit && (
			...
			Messages.error("error_invalid_region");
			return false;
		}
		...
		if (options.projtype === 1 && options.projection === "") {
			Messages.error("error_undefined_projection");
			return false;
		}
	...
	if (radioValue(form.gpxsource) === 0) {
		...
		if (files.length === 0) {
			Messages.error('error_no_gpx_selected');
			return;
		}
...
var loader = function (options, gpx_url) {
	...
	// Check if user wants to use trimmed GPX data
	if (shouldUseTrimmedGpx && shouldUseTrimmedGpx()) {
		var trimmedPoints = getTrimmedGpxPoints();
		if (trimmedPoints.length >= 2) {
			Messages.status("使用截取的GPX路径段 (" + trimmedPoints.length + " 个点)");
			g = new Gpex(options, trimmedPoints);
			return;
		} else {
			Messages.error("截取的路径段点数太少，至少需要2个点");
			return;
		}
	}
...
Basemap.prototype.Update = function (bounds, bed) {
	...
	var mapurl = "https://maps.googleapis.com/maps/api/staticmap?"
		+ "center=" + center[1].toFixed(6) + "," + center[0].toFixed(6)
		+ "&zoom=" + zoominfo.zoom
		+ "&size=" + mapsize.width + "x" + mapsize.height
		+ "&maptype=hybrid"
		+ "&scale=2"
		+ "&format=jpg"
		+ "&key=AIzaSyBMTdBdNXMyAWYU8Sn4dt4WYtsf5lqvldA";

	if (this.view !== null) {
		// 使用 GM_xmlhttpRequest 获取 blob，不发送 Referer/Origin
		GM_xmlhttpRequest({
			method: 'GET',
			url: mapurl,
			responseType: 'blob',
			// 不设置 headers，油猴默认不会发送 Referer/Origin
			onload: (res) => {
				const imgUrl = URL.createObjectURL(res.response);
				this.view.setBaseMap(imgUrl, mapscale, bed.x, bed.y, this.Download);
			},
			onerror: (err) => {
				console.error('Google Maps 静态图加载失败', err);
			}
		});
	}
...
var Parser = {
	...
	track: function (track) {
		...
		if (segments.length === 0) {
			Messages.error("error_no_track_segments");
			return null;
		}
		...
		if (pts.length < 2) {
			Messages.error('error_insufficient_track_points');
			return null;
		}
